...
- **mlx-lm 检测可靠性修复**：设置页和仪表盘现在读取已安装 `mlx-lm` 的包元数据，不再通过完整导入判断安装状态，避免已安装环境被误显示为未安装。
- **环境配置验证**：环境配置流程现在会在显示成功前验证已安装的 `mlx-lm` 版本。
...
### 轻松微调 (LoRA)
- **统一模型库**：自动检测本地 HuggingFace / ModelScope / Ollama 资产，或在线拉取最新模型（Qwen、DeepSeek、GLM、Gemma、Llama、GPT-OSS 等）。
